...
uint8_t esp_ble::begin()
{
    ...
    pic_service_p = pServer->createService(PIC_SERVICE_UUID);
    esp_service_p = pServer->createService(ESP_SERVICE_UUID);

    // NEW:
    tools_p = esp_service_p->createCharacteristic(TOOLS_CH_UUID, NIMBLE_PROPERTY::READ | NIMBLE_PROPERTY::WRITE | NIMBLE_PROPERTY::NOTIFY);
    keys_p = esp_service_p->createCharacteristic(KEYS_CH_UUID, NIMBLE_PROPERTY::READ);
    ota_p = esp_service_p->createCharacteristic(OTA_CH_UUID, NIMBLE_PROPERTY::NOTIFY);
    bl_p = pic_service_p->createCharacteristic(BACKLIGHT_CH_UUID, NIMBLE_PROPERTY::WRITE);
    app_read_p = pic_service_p->createCharacteristic(APP_READ_CH_UUID, NIMBLE_PROPERTY::READ | NIMBLE_PROPERTY::NOTIFY);
    app_cal_p = pic_service_p->createCharacteristic(APP_CAL_CH_UUID, NIMBLE_PROPERTY::READ | NIMBLE_PROPERTY::WRITE | NIMBLE_PROPERTY::NOTIFY);
    app_reg_p = pic_service_p->createCharacteristic(APP_REG_CH_UUID, NIMBLE_PROPERTY::READ | NIMBLE_PROPERTY::WRITE | NIMBLE_PROPERTY::NOTIFY);
...
uint8_t esp_ble::stop()
{
    ...
    NimBLEDevice::getServer()->removeService(pic_service_p);
    NimBLEDevice::getServer()->removeService(esp_service_p);
    delay(10);
...
void tools_cb::onSubscribe(NimBLECharacteristic *p, ble_gap_conn_desc *desc, uint16_t sub_val)
{
    if(sub_val)
        Serial.println("(ISR) SUBSCRIPTION SET BY BLE DEVICE!");
}
//-----------------------------------------------//
