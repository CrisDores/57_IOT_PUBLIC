...
uint8_t uart_comms::begin()
{
    ...
    for (uint8_t attempts = 0x00; attempts < m.retry_count; attempts++) // SET CONNECTION STATUS
    {
        uint8_t scode = set_pic_connection_status(m.cstats[0], m.cstats[1], m.cstats[2]);
        delay(5);
    ...
    Serial.println("BEGIN UART: SET CONNECTION STATUS SUCCESS");

    for (uint8_t attempts = 0x00; attempts < m.retry_count; attempts++) // GET PIC WORK DATA
    {
        uint8_t scode = get_pic_work_data(&m.work_data[0], m.work_data_len);
        delay(5);
    ...
    Serial.println("BEGIN UART: GET WORK DATA SUCCESS");

    for (uint8_t attempts = 0x00; attempts < m.retry_count; attempts++) // GET PIC CAL DATA
    {
        uint8_t scode = get_pic_calibration_data(&m.calibration_data[0], m.calibration_data_len);
        delay(5);
    ...
    Serial.println("BEGIN UART: GET CALIBRATION DATA SUCCESS");

    for (uint8_t attempts = 0x00; attempts < m.retry_count; attempts++) // GET PIC REG DATA
    {
        uint8_t scode = get_pic_regulation_data(&m.regulation_data[0], m.regulation_data_len);
        delay(5);
    ...
    Serial.println("BEGIN UART: GET REGULATION DATA SUCCESS");

    Serial.println("BEGIN UART SUCCESS");
...
uint8_t uart_comms::uart_byte_comm(uint8_t *cmd, uint8_t *rply, size_t len)
{
    ...
    Serial.println();
    Serial.println();
    Serial.print("RECEIVING BYTES FROM PIC: ");
...
uint8_t uart_comms::get_pic_calibration_data(uint8_t *data, uint8_t data_len) // DATA ARR: 10
{
    manager &m = manager::getInstance(); // invoke singleton instance
    ...
    cmd[0] = HEADERS;
    cmd[1] = 0xA1; // get calibration data command
    cmd[2] = 0x00;
    cmd[3] = 0x00;
    cmd[4] = 0x00;
    cmd[5] = 0x00;
    ...
    uint8_t payload[rply[2]];      // VLA seems to work fine
    uint8_t payload_len = rply[2]; // for VLAs this works fine
    ...
    for (uint8_t i = 0; i < data_len; i++)
        data[i] = payload[i + 2]; // to the args

    //////////////////

    return 0x00; // SUCCESS
}
uint8_t uart_comms::set_pic_calibration_data(uint8_t datal, uint8_t datah, uint8_t cp)
{
    uint8_t cmd[8] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}; // to PIC
    uint8_t cmd_len = sizeof(cmd) / sizeof(cmd[0]);
    ...
    cmd[0] = HEADERS;
    cmd[1] = 0xA2;  // set cal cmd
    cmd[2] = cp;    // calibration point
    cmd[3] = datal; // data low
    cmd[4] = datah; // data high
    cmd[5] = 0x00;
    cmd[7] = HEADERS;
...
uint8_t uart_comms::get_pic_regulation_data(uint8_t *data_arr, size_t data_len) // DATA: 15 // PRIVATE USE ONLY
{
    uint8_t cmd[8] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}; // to PIC
    uint8_t cmd_len = sizeof(cmd) / sizeof(cmd[0]);
    ...
    cmd[0] = HEADERS;
    cmd[1] = 0xB1; // get regulation data command
    cmd[2] = 0x00;
    cmd[3] = 0x00;
    cmd[4] = 0x00;
    cmd[5] = 0x00;
    ...
    for (uint8_t i = 0; i < data_len; i++)
        data_arr[i] = payload[i + 2]; // to the args

    return 0x00; // SUCCESS
}
uint8_t uart_comms::set_pic_regulation_data(uint8_t datal, uint8_t datah, uint8_t rp) // PRIVATE USE ONLY
{
    uint8_t cmd[8] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}; // to PIC
    uint8_t cmd_len = sizeof(cmd) / sizeof(cmd[0]);
    ...
    cmd[0] = HEADERS;
    cmd[1] = 0xB2;  // set reg cmd
    cmd[2] = rp;    // regulation point
    cmd[3] = datal; // data low
    cmd[4] = datah; // data high
    cmd[5] = 0x00;
    cmd[7] = HEADERS;
